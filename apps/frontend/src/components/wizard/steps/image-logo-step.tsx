...
export function ImageLogoStep() {
  ...
  const requestSelectedImagesGeneration = async () => {
    if (selectedImages.size === 0) {
      alert('Veuillez sélectionner au moins une image à générer');
      return;
    }
      ...
      <div className="wizard-step p-8">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-blue-900 mb-2">
            {aiRequest.status === 'pending' ? '⏳ Demande envoyée' : '🎨 Génération en cours...'}
          </h3>
          <p className="text-blue-800">
            {aiRequest.status === 'pending'
              ? 'Votre demande est en attente de traitement par l\'administrateur'
              : 'Les images sont en cours de génération par IA'}
          </p>
          {aiRequest.elapsedTime && (
            <p className="text-sm text-blue-600 mt-2">
              ⏱️ Temps écoulé: {aiRequest.elapsedTime}
            </p>
          )}
        </div>
      </div>
    ...
    <div className="wizard-step p-8">
      ...
      <div className="mb-6 flex gap-4">
        {!selectiveMode ? (
          <>
            ...
            >
              🎨 Générer TOUTES mes images par IA
            </button>
            <button
            ...
            >
              ✅ Générer CERTAINES images par IA
            </button>
          </>
        ) : (
          <>
            ...
            >
              🚀 Générer les {selectedImages.size} image(s) sélectionnée(s)
            </button>
            <button
              onClick={() => {
                setSelectiveMode(false);
      ...
      {selectiveMode && (
        <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm text-yellow-900">
            <strong>Mode sélectif :</strong> Cochez les images que vous souhaitez générer par IA, puis cliquez sur "Générer".
          </p>
        </div>
      )}

      {/* Colors Section - Above Images */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="border-2 border-blue-200 rounded-lg p-4 bg-blue-50">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              />
              <span className="text-gray-900 font-mono text-sm font-semibold">{wizardData.colors?.primary || '#4F46E5'}</span>
            </div>
          ...
          <div className="border-2 border-green-200 rounded-lg p-4 bg-green-50">
            ...
            <div className="flex items-center gap-3 mb-2">
              <input
                ...
                className="w-24 h-12 rounded-lg border-2 border-gray-300 cursor-pointer shadow-sm"
              />
              <span className="text-gray-900 font-mono text-sm font-semibold">{wizardData.colors?.secondary || '#10B981'}</span>
            </div>
          ...
          <div className="border-2 border-purple-200 rounded-lg p-4 bg-purple-50">
            ...
            <div className="flex items-center gap-3 mb-2">
              <input
                ...
                className="w-24 h-12 rounded-lg border-2 border-gray-300 cursor-pointer shadow-sm"
              />
              <span className="text-gray-900 font-mono text-sm font-semibold">{wizardData.colors?.accent || '#A78BFA'}</span>
            </div>
      ...
      <div className="space-y-6 border border-gray-300 rounded-lg p-6 bg-white">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {renderImageField('Hero Image', 'hero', heroInputRef, '🌄')}
            {renderImageField('Logo Navigation', 'logo', logoInputRef, '📱')}
            {renderImageField('Logo Footer', 'logoFooter', logoFooterInputRef, '🦶')}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {renderImageField('Favicon Clair', 'faviconLight', faviconLightInputRef, '☀️')}
            {renderImageField('Favicon Sombre', 'faviconDark', faviconDarkInputRef, '🌙')}
          </div>
        </div>
      ...
      {aiRequest?.status === 'completed' && (
        <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4">
          <h4 className="font-semibold text-green-900 mb-2">✅ Images générées avec succès!</h4>
          <p className="text-green-800 text-sm">
            Vos images ont été générées et sont maintenant disponibles ci-dessus.
          </p>
        </div>
      )}
      ...
      {(aiRequest?.status === 'rejected' || aiRequest?.status === 'failed') && (
        <div className="mt-6 bg-red-50 border border-red-200 rounded-lg p-4">
          <h4 className="font-semibold text-red-900 mb-2">
            {aiRequest.status === 'rejected' ? '❌ Demande rejetée' : '⚠️ Erreur de génération'}
          </h4>
          <p className="text-red-800 text-sm">
            {aiRequest.errorMessage || 'Une erreur est survenue lors de la génération des images.'}
          </p>
        </div>
      )}
    </div>
