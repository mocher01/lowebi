...
const Navbar = () => {
  ...
  // 🚀 FIX v1.0.5: Fonction pour gérer la navigation avec scroll vers le haut
  const handleHomeNavigation = (e) => {
    e.preventDefault();
    
    // Si on est déjà sur la home page, scroll vers le haut
    if (location.pathname === '/') {
      ...
      // Sinon, naviguer vers la home page (ScrollToTop component s'occupe du scroll)
      navigate('/');
    }
  ...
  const navbarAccent = configWithAutoColors.navbar?.accentColor || config.brand?.colors?.primary || "#3B82F6";
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          <div className="flex flex-col items-start leading-tight">
            ...
            {config.brand?.showSloganBelowLogo !== false && (
              <span 
                className="text-sm sm:text-[13px] font-medium tracking-tight -mt-1 pl-[10px]"
                style={{ color: `${navbarText}cc` }}
              >
                {config.brand.slogan}
              </span>
            )}
        ...
        >
          <div className="container mx-auto px-4 py-4">
            <nav className="flex flex-col space-y-4">
              ...
              <Link to={ctaConfig.path} onClick={() => setIsOpen(false)}>
                <Button 
