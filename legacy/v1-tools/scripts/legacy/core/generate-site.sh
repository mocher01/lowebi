...
if [ -d "$CONTENT_DIR" ]; then
    ...
    if [ -d "$OUTPUT_DIR/public/content/blog" ]; then
        ...
        node scripts/maintenance/generate-blog-index.js "$OUTPUT_DIR/public/content/blog" "$OUTPUT_DIR/public/content/blog-index.json"
    fi
else
...
if [ -f "scripts/maintenance/inject-html-config.js" ]; then
    node scripts/maintenance/inject-html-config.js "$CONFIG_FILE" "$OUTPUT_DIR"
else
    echo -e "${YELLOW}⚠️  Warning: inject-html-config.js not found${NC}"
fi

# 🔧 RESTAURER: Génération CSS Variables Layout (Architecture Clean v1.1)
echo "  🎨 Generating Layout CSS Variables..."
if [ -f "scripts/maintenance/generate-layout-css.js" ]; then
    node scripts/maintenance/generate-layout-css.js "$CONFIG_FILE" "$OUTPUT_DIR"
else
    echo -e "${YELLOW}⚠️  Warning: generate-layout-css.js not found${NC}"
fi
...
if command -v node &> /dev/null; then
    if [ -f "scripts/utils/logo-config-optimizer.js" ]; then
        ...
        node scripts/utils/logo-config-optimizer.js "$CONFIG_FILE" || true
...
if grep -q '"enabled".*:.*true' "$CONFIG_FILE" 2>/dev/null && grep -q '"n8n"' "$CONFIG_FILE" 2>/dev/null; then
    ...
    if command -v node &> /dev/null; then
        ...
        if node scripts/generators/n8n-workflow-generator.js "$SITE_NAME" "$CONFIG_FILE"; then
            ...
            echo "   node scripts/generators/n8n-workflow-generator.js $SITE_NAME $CONFIG_FILE"
        fi
    else
...
if [ "$N8N_ENABLED" = "true" ] && command -v node &> /dev/null; then
    ...
    if node scripts/maintenance/inject-nginx-webhook.js "$SITE_NAME"; then
...
if [ "$IMAGE_MODE" != "none" ]; then
    ...
    IMAGE_MODE="$IMAGE_MODE" FORCE_REGENERATE="$FORCE_REGENERATE" FORCE_BLOG_IMAGES="$FORCE_BLOG_IMAGES" FORCE_HERO_IMAGES="$FORCE_HERO_IMAGES" FORCE_SERVICE_IMAGES="$FORCE_SERVICE_IMAGES" node scripts/generators/image-controller.js "$CONFIG_FILE" "$OUTPUT_DIR/public/assets"
    ...
    ASSETS_SOURCE="configs/$SITE_NAME/assets"
    ...
    if [ -d "$ASSETS_SOURCE" ]; then
        ...
        # Copier tous les assets sans traitement
