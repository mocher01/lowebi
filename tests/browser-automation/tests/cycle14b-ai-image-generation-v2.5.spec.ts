...
test('Cycle 14b V2.5: Correct Image-to-Field Mapping', async ({ page, browser }) => {
  test.setTimeout(600000); // 10 minutes timeout for complete workflow
  console.log('🔄 CYCLE 14b V2.5: CORRECT IMAGE-TO-FIELD MAPPING');
  console.log('Purpose: Ensure each image type goes to the correct field');
  console.log('V2.5 Improvements:');
  console.log('  - Dynamically detect field labels');
  console.log('  - Upload service images to actual service fields');
  console.log('  - Upload blog images to actual blog article fields');
  console.log('  - Ensure NO empty fields remain');
  console.log('Flow: Foundation (1-6) → Content AI (10-13) → Image AI (14b) → SMART UPLOAD → VERIFICATION');
  ...
  try {
    ...
    const siteName = `Cycle14b_V2.5_${timestamp}`;
