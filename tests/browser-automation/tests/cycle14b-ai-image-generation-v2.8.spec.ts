...
test('Cycle 14b V2.8: Content Persistence + Step 6 Feature Cards', async ({ page, browser }) => {
  test.setTimeout(600000); // 10 minutes timeout for complete workflow
  console.log('🔄 CYCLE 14b V2.8: CONTENT PERSISTENCE + STEP 6 FEATURE CARDS');
  console.log('Purpose: Verify content persists after image AI processing + Test new Step 6 UI');
  console.log('V2.8 Improvements:');
  console.log('  - All V2.7 regression fixes (content persistence)');
  console.log('  - NEW: Verify Step 6 Feature Cards UI (Email Configuration)');
  console.log('  - NEW: Test 3 email scenarios (OAuth2, Locod.ai, No Form)');
  console.log('  - NEW: Test dynamic states and dependencies');
  console.log('Flow: Foundation (1-5) → Step 6 (FEATURE CARDS TEST) → Content AI (10-13) → Image AI (14b)');
  ...
  try {
    ...
    const siteName = `Cycle14b_V2.8_${timestamp}`;
    ...
    console.log('✅ FOUNDATION STEPS 1-5 COMPLETED');

    // ============================================================================
    // V2.8 NEW: NAVIGATE TO STEP 6 AND TEST FEATURE CARDS
    // ============================================================================
    console.log('\n🎯 STEP 6: NAVIGATE TO ADVANCED FEATURES & TEST FEATURE CARDS (V2.8 NEW)');
    console.log('='.repeat(80));
    ...
    let step6Results = {
      ...
      gdprConsentTested: false,
      dynamicStatesTested: false
    };

    try {
      // Navigate from Step 5 to Step 6
      console.log('\n➡️ Clicking Suivant to navigate from Step 5 to Step 6...');
      ...
      if (suivantCount > 0) {
        ...
        if (isEnabled) {
          ...
          console.log('⚠️ Suivant button is disabled - skipping Step 6 test');
        }
      } else {
        console.log('⚠️ No Suivant button found - skipping Step 6 test');
      }
      ...
      if (step6Title > 0) {
        ...
        console.log('\n📊 STEP 6 TEST SUMMARY');
        console.log('-'.repeat(50));
        const totalTests = Object.values(step6Results).filter(v => v === true).length;
        const totalPossible = Object.keys(step6Results).length;
        console.log(`✅ Tests Passed: ${totalTests}/${totalPossible}`);

        if (totalTests >= 7) {
          console.log('🎉 STEP 6 FEATURE CARDS UI WORKING!');
        ...
        // Navigate back to Step 5 for content generation
        console.log('\n⬅️ Navigating back to Step 5 for AI content generation...');
        const backButton = page.locator('button:has-text("Retour"), button:has-text("Précédent")');
        const backCount = await backButton.count();
        if (backCount > 0) {
          await backButton.first().click();
          await page.waitForTimeout(2000);
          console.log('✅ Returned to Step 5');
        }

      } else {
        console.log('⚠️ Not on Step 6 - Feature Cards test skipped');
      ...
      console.log('⚠️ Continuing with rest of test...');
    }
      ...
      if (step5Verified) {
        ...
        if (finalSuivantCount > 0) {
          ...
          if (isEnabled) {
            ...
            await page.waitForTimeout(2000);
            console.log('✅ Clicked Suivant - Proceeding to Step 6');
          } else {
    ...
    // ============================================================================
    // V2.8 NEW: STEP 6 FEATURE CARDS TEST
    // ============================================================================
    console.log('\n🎯 STEP 6: FEATURE CARDS UI TEST (V2.8 NEW)');
    console.log('='.repeat(80));
    ...
    try {
      ...
      if (step6Title > 0) {
        console.log('✅ On Step 6: Fonctionnalités avancées');
        step6Verified = true;
        ...
        // Test 4: Test Locod.ai Scenario (GDPR Consent)
        console.log('\n🧪 TEST 4: Locod.ai Scenario - GDPR Consent');
        console.log('-'.repeat(50));

        if (locodaiRadio > 0) {
          await page.locator('input[value="locodai-default"]').click();
          await page.waitForTimeout(1000);
          console.log('✅ Selected Locod.ai scenario');

          const gdprConsent = await page.locator('text=Consentement GDPR requis').count();
          console.log(`📝 GDPR Consent Form: ${gdprConsent > 0 ? 'VISIBLE ✅' : 'NOT VISIBLE ❌'}`);
          ...
          if (gdprConsent > 0) {
            ...
            if (emailInput > 0) {
              ...
              step6Results.gdprConsentTested = consentAccepted > 0;
              console.log(`✅ GDPR Consent: ${consentAccepted > 0 ? 'ACCEPTED ✅' : 'NOT ACCEPTED ❌'}`);
            }
          }
        }

        // Test 5: Test Dynamic States (OAuth2 vs Locod.ai vs No Form)
        console.log('\n🧪 TEST 5: Dynamic States');
        console.log('-'.repeat(50));

        // Test "No Form" scenario
        if (noFormRadio > 0) {
          await page.locator('input[value="no-form"]').click();
          await page.waitForTimeout(1000);
          console.log('✅ Selected "No Form" scenario');

          const blockedMessage = await page.locator('text=pas de formulaire').count();
          console.log(`🚫 Blocked state message: ${blockedMessage > 0 ? 'SHOWN ✅' : 'NOT SHOWN ❌'}`);
          step6Results.dynamicStatesTested = blockedMessage > 0;
        }

        // Return to OAuth2 for next steps
        if (oauth2Radio > 0) {
          await page.locator('input[value="oauth2"]').click();
          await page.waitForTimeout(1000);
          console.log('✅ Returned to OAuth2 scenario for next steps');
        }
        ...
        const totalTests = Object.values(step6Results).filter(v => v === true).length;
        const totalPossible = Object.keys(step6Results).length;
        console.log(`✅ Passed: ${totalTests}/${totalPossible} tests`);

        if (totalTests === totalPossible) {
          console.log('🎉 ALL STEP 6 TESTS PASSED!');
        } else if (totalTests >= 7) {
          console.log('✅ STEP 6 CORE FUNCTIONALITY WORKING');
        } else {
          console.log('⚠️ STEP 6 PARTIAL: Some features missing');
        }

      } else {
        console.log('⚠️ Not on Step 6 yet - Step 6 test skipped');
        console.log('Current page title:', await page.locator('h2').first().textContent());
      }

    } catch (error) {
      console.error('❌ Step 6 test error:', error);
      console.log('⚠️ Step 6 test failed but continuing with rest of test');
    }

    // ============================================================================
    // FINAL V2.8 RESULTS (Session Storage + Step 5 + Step 6 Verification)
    // ============================================================================
    console.log('\n🎉 CYCLE 14b V2.8 COMPLETE - SESSION STORAGE + STEP 5 + STEP 6');
    console.log('=' .repeat(80));
    ...
    const finalResults = {
      ...
      // V2.8 NEW: Step 6 results
      step6Navigation: step6Verified,
      step6EmailCard: step6Results.emailCardFound,
      step6ThreeScenarios: step6Results.oauth2RadioFound && step6Results.locodaiRadioFound && step6Results.noFormRadioFound,
      step6FeatureCards: step6Results.n8nCardFound && step6Results.analyticsCardFound && step6Results.recaptchaCardFound,
      step6GDPRConsent: step6Results.gdprConsentTested,
      step6DynamicStates: step6Results.dynamicStatesTested
    };
    ...
    // V2.8 NEW: Step 6 results
    console.log('\n🆕 V2.8 NEW: Step 6 Feature Cards Tests');
    console.log(`${finalResults.step6Navigation ? '✅' : '❌'} Step 6 Navigation: ${finalResults.step6Navigation ? 'PASSED' : 'FAILED'}`);
    console.log(`${finalResults.step6EmailCard ? '✅' : '❌'} Step 6 Email Card: ${finalResults.step6EmailCard ? 'PASSED' : 'FAILED'}`);
    console.log(`${finalResults.step6ThreeScenarios ? '✅' : '❌'} Step 6 Three Scenarios: ${finalResults.step6ThreeScenarios ? 'PASSED' : 'FAILED'}`);
    console.log(`${finalResults.step6FeatureCards ? '✅' : '❌'} Step 6 Feature Cards (N8N/Analytics/reCAPTCHA): ${finalResults.step6FeatureCards ? 'PASSED' : 'FAILED'}`);
    console.log(`${finalResults.step6GDPRConsent ? '✅' : '❌'} Step 6 GDPR Consent: ${finalResults.step6GDPRConsent ? 'PASSED' : 'FAILED'}`);
    console.log(`${finalResults.step6DynamicStates ? '✅' : '❌'} Step 6 Dynamic States: ${finalResults.step6DynamicStates ? 'PASSED' : 'FAILED'}`);

    const allTestsPassed = Object.values(finalResults).every(result => result === true);
    const step6TestsPassed = finalResults.step6Navigation && finalResults.step6EmailCard && finalResults.step6ThreeScenarios;

    if (allTestsPassed) {
      console.log('\n🎉 🎉 🎉 COMPLETE SUCCESS: ALL TESTS PASSED! 🎉 🎉 🎉');
      console.log('✅ V2.7: Session storage for unified image management');
      console.log('✅ V2.8: Step 6 Feature Cards UI fully functional');
      console.log('✅ Complete end-to-end workflow functional');
    } else if (step6TestsPassed) {
      console.log('\n🎉 STEP 6 SUCCESS: New Feature Cards UI Working!');
      console.log('✅ V2.8: Step 6 Feature Cards implemented and tested');
      console.log('⚠️ Some V2.7 tests still need attention');
    } else {
      console.log('\n⚠️ PARTIAL SUCCESS: Some components need attention');
      console.log('📊 V2.7 session storage architecture is operational');
      if (!step6TestsPassed) {
        console.log('⚠️ V2.8 Step 6 tests need review');
      }
    }

    console.log('\n📊 V2.8 SUMMARY:');
    console.log('✅ V2.7: Session storage: /uploads/sessions/{sessionId}/');
    console.log('✅ V2.7: Polling fetches images after admin processing');
    console.log('✅ V2.7: Images auto-apply to wizardData.images');
    console.log('✅ V2.7: Step 5 displays generated images with download/replace');
    console.log('✅ V2.7: Blog images now shown in AI section');
    console.log('🆕 V2.8: Step 6 Feature Cards UI with Email Configuration');
    console.log('🆕 V2.8: Three email scenarios (OAuth2, Locod.ai, No Form)');
    console.log('🆕 V2.8: Dynamic states and dependencies tested');
    console.log('🆕 V2.8: GDPR consent flow verified');
    console.log('💡 Implements Phase 1 from Issue #129');
