...
test('Cycle 14b V2.3: Session Storage + Step 5 Verification', async ({ page, browser }) => {
  ...
  try {
    ...
    const siteName = `Cycle14b_V2.3_${timestamp}`;
    ...
    // After clicking Continue, we should be at the wizard step where we left off
    console.log('\n➡️ V2.3: Waiting for wizard to load after Continue click...');
    ...
    // Check if we're on the Images step (V2.3 unified design)
    ...
    if (imagesOptionCount > 0) {
      console.log('✅ Continue button took us to Images step! (V2.3 unified design)');
      navigationSuccess = true;
    } else {
      console.log('⚠️ Not on Images step after Continue. Trying to click Suivant...');
      ...
      if (suivantCount > 0) {
        ...
        if (isEnabled) {
          ...
          const afterClick = await page.locator('text="Générer TOUTES mes images par IA"').count();
          if (afterClick > 0) {
            console.log('✅ Reached Images step after Suivant click!');
            navigationSuccess = true;
          }
        }
      }
