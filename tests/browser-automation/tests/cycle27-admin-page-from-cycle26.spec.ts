...
test('Cycle 27: Admin Manage Page E2E Test (Based on Cycle 26)', async ({ page, browser }) => {
  test.setTimeout(900000); // 15 minutes timeout for complete workflow + admin testing
  console.log('🔄 CYCLE 27: ADMIN MANAGE PAGE E2E TEST (BASED ON CYCLE 26)');
  console.log('Purpose: Test Issue #168 Phase 1 - Admin Dashboard Foundation + Site Preview');
  console.log('Cycle 27 Goals:');
  console.log('  - Complete all foundation steps (1-5)');
  console.log('  - Test domain selector in Step 2 (Issue #153)');
  console.log('  - Test subdomain creation with wizard session');
  ...
  console.log('  - Verify domain display in Step 7 (Issue #153)');
  console.log('  - GENERATE SITE with domain creation (Issue #153)');
  console.log('  - Verify HTTPS subdomain access with wildcard SSL (Issue #153)');
  console.log('  - NEW: Verify "Manage" button appears in MySites for deployed sites (Issue #168)');
  console.log('  - NEW: Click "Manage" and navigate to /admin/sites/[id] (Issue #168)');
  console.log('  - NEW: Verify NO 401 Unauthorized errors on admin dashboard (CRITICAL!)');
  console.log('  - NEW: Verify admin dashboard components (preview, quick actions, logs)');
  console.log('Flow: Foundation → Domain → Content AI → Image AI → Step 6 → Step 7 → Generate → HTTPS → Admin Dashboard');
  console.log('=' .repeat(80));

  // Enable console logging to capture blog content application and generation
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('blog') || text.includes('Blog') || text.includes('401') || text.includes('Unauthorized') || text.includes('admin') || text.includes('Manage') || text.includes('deployment')) {
  ...
  try {
    ...
    const step13PageContent = await page.textContent('body');
    ...
    for (const content of expectedContent) {
      if (step13PageContent.includes(content)) {
    ...
    try {
      ...
      if (!generateButtonFound) {
        ...
        const generationPageContent = await page.textContent('body').catch(() => '');
        if (generationPageContent.includes('génération') || generationPageContent.includes('Génération') || generationPageContent.includes('progress')) {
    ...
    // ============================================================================
    // STEPS 27-32: ADMIN/MANAGE PAGE TESTING (ISSUE #168 PHASE 1)
    // ============================================================================

    console.log('\n🎯 STEPS 27-32: ADMIN/MANAGE PAGE TESTING (ISSUE #168 PHASE 1)');
    console.log('='.repeat(70));

    // STEP 27: Navigate to MySites page
    console.log('🏠 Step 27: Navigate to MySites page...');
    await page.goto('https://dev.lowebi.com/sites');
    await page.waitForTimeout(5000);

    console.log('✅ On MySites page');

    // STEP 28: Find and verify the site in the list
    console.log('🔍 Step 28: Find and verify site in list...');
    ...
    // STEP 29: Verify "Manage" button is visible (NEW FEATURE)
    console.log('🔍 Step 29: Verify "Manage" button is visible (NEW FEATURE - Issue #168)...');
    ...
    if (manageButtonExists === 0) {
      console.log('⚠️ "Manage" button not found - checking deployment status...');

      // Wait additional time for deployment to complete
      console.log('⏳ Waiting additional 30 seconds for deployment status update...');
      ...
      if (manageButtonRetry === 0) {
        throw new Error('"Manage" button not visible - site may not be marked as deployed');
      }
    }

    console.log('✅ "Manage" button is visible for deployed site');

    // STEP 30: Click "Manage" button and navigate to admin dashboard (CRITICAL TEST)
    console.log('🎯 Step 30: Click "Manage" button and navigate to admin dashboard...');
    ...
    // STEP 31: Verify admin dashboard loaded without 401 errors
    console.log('🔍 Step 31: Verify admin dashboard loaded without 401 errors...');
    ...
    if (adminPageContent.includes('401') || adminPageContent.includes('Unauthorized')) {
      console.log('❌ CRITICAL ERROR: 401 Unauthorized detected on admin page!');
      console.log('Page content:', adminPageContent.substring(0, 500));
    ...
    // STEP 32: Verify admin dashboard components are present
    console.log('🔍 Step 32: Verify admin dashboard components are present...');
    ...
    // ============================================================================
    // STEP 33: TEST QUICK ACTIONS (OPTIONAL - LOGS)
    // ============================================================================

    console.log('\n🎯 STEP 33: TEST QUICK ACTIONS (VIEW LOGS)');
    console.log('='.repeat(60));
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🎉 CYCLE 27 COMPLETE: Domain Integration + Admin Manage Page E2E Test');
    console.log('='.repeat(80));
