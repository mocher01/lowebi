...
test('Cycle 14b V2.7: Content Persistence After Image Processing', async ({ page, browser }) => {
  test.setTimeout(600000); // 10 minutes timeout for complete workflow
  console.log('🔄 CYCLE 14b V2.7: CONTENT PERSISTENCE AFTER IMAGE PROCESSING');
  console.log('Purpose: Verify content persists after image AI processing (regression fix)');
  console.log('V2.7 Improvements:');
  console.log('  - CRITICAL: Validate Principal tab content AFTER image processing');
  console.log('  - CRITICAL: Ensure hero title/subtitle/description NOT empty');
  console.log('  - Verify image upload and persistence');
  console.log('  - Check admin UI "Images uploadées" counter');
  console.log('Flow: Foundation (1-6) → Content AI (10-13) → Content Check #1 → Image AI (14b) → Content Check #2 (REGRESSION FIX)');
  ...
  try {
    ...
    const siteName = `Cycle14b_V2.7_${timestamp}`;
