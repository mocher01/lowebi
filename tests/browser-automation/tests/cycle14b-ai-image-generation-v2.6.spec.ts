...
test('Cycle 14b V2.6: Upload Verification + Proper Image Mapping', async ({ page, browser }) => {
  test.setTimeout(600000); // 10 minutes timeout for complete workflow
  console.log('🔄 CYCLE 14b V2.6: UPLOAD VERIFICATION + PROPER IMAGE MAPPING');
  console.log('Purpose: Verify ALL images actually upload and persist');
  console.log('V2.6 Improvements:');
  console.log('  - FIX: Verify each upload with image preview detection');
  console.log('  - FIX: Check admin UI "Images uploadées" counter');
  console.log('  - FIX: Longer wait times for draft save to complete');
  console.log('  - Smart label detection for service/blog fields');
  console.log('Flow: Foundation (1-6) → Content AI (10-13) → Image AI (14b) → VERIFIED UPLOAD → COUNTER CHECK');
  ...
  try {
    ...
    const siteName = `Cycle14b_V2.6_${timestamp}`;
