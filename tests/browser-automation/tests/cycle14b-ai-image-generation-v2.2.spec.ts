...
test('Cycle 14b V2.2: Image Display & Download Verification', async ({ page, browser }) => {
  console.log('🔄 CYCLE 14b V2.2: IMAGE DISPLAY & DOWNLOAD VERIFICATION');
  console.log('Purpose: Execute Steps 1-13 + Step 14b + VERIFY IMAGE DISPLAY IN ADMIN');
  console.log('V2.2 Addition: Verify images are displayed (not just "✓ Uploadé" text) with download buttons');
  console.log('Flow: Foundation (1-6) → Content AI (10-13) → Image AI (14b) → DISPLAY CHECK');
  ...
  try {
    ...
    const siteName = `Cycle14b_V2.1_${timestamp}`;
    ...
    // === FILE UPLOAD WORKFLOW - Upload real images ===
    console.log('🖼️ Using FILE UPLOAD workflow - uploading real image files...');

      // Prepare test images paths
      const testImagesDir = path.join(__dirname, '..', 'test-images');
      const imageFiles = {
        logo: path.join(testImagesDir, 'logo.png'),
        logoFooter: path.join(testImagesDir, 'logo.png'), // reuse logo for footer
        hero: path.join(testImagesDir, 'hero.png'),
        blog: path.join(testImagesDir, 'blog-1.png'),
        about: path.join(testImagesDir, 'service-1.png'),
        contact: path.join(testImagesDir, 'service-2.png'),
        faviconLight: path.join(testImagesDir, 'favicon.png'),
        faviconDark: path.join(testImagesDir, 'favicon.png'),
        ...
        service4: path.join(testImagesDir, 'service-1.png'), // reuse
        service5: path.join(testImagesDir, 'service-2.png'), // reuse
        blog1: path.join(testImagesDir, 'blog-1.png'),
        blog2: path.join(testImagesDir, 'blog-2.png'),
        blog3: path.join(testImagesDir, 'blog-3.png')
      };

      // STEP 5: Upload real image files
      console.log('\n📤 Step 5: Uploading real image files to each upload field...');

      const fileInputs = imageAdminPage.locator('input[type="file"]');
      const fileInputCount = await fileInputs.count();
      console.log(`🔍 Found ${fileInputCount} file upload fields`);

      // Map of expected upload fields to image files
      const uploadMapping = [
        { label: 'Logo Navigation', file: imageFiles.logo },
        { label: 'Logo Footer', file: imageFiles.logoFooter },
        { label: 'Hero', file: imageFiles.hero },
        { label: 'Blog', file: imageFiles.blog },
        { label: 'À Propos', file: imageFiles.about },
        { label: 'Contact', file: imageFiles.contact },
        { label: 'Favicon Clair', file: imageFiles.faviconLight },
        { label: 'Favicon Sombre', file: imageFiles.faviconDark },
        { label: 'Service - Conseil', file: imageFiles.service1 },
        { label: 'Service - Solutions', file: imageFiles.service2 },
        { label: 'Service - Support', file: imageFiles.service3 },
        { label: 'Service - Formation', file: imageFiles.service4 },
        { label: 'Service - Optimisation', file: imageFiles.service5 },
        { label: 'Blog Article 1', file: imageFiles.blog1 },
        { label: 'Blog Article 2', file: imageFiles.blog2 },
        { label: 'Blog Article 3', file: imageFiles.blog3 }
      ];

      // Upload images to each field
      let uploadedCount = 0;
      for (let i = 0; i < Math.min(fileInputCount, uploadMapping.length); i++) {
        try {
          const fileInput = fileInputs.nth(i);
          const mapping = uploadMapping[i];
          ...
          // Upload the file
          await fileInput.setInputFiles(mapping.file);
          uploadedCount++;
          console.log(`✅ Uploaded ${path.basename(mapping.file)} to field ${i + 1} (${mapping.label})`);
          await imageAdminPage.waitForTimeout(500); // Small delay between uploads
        } catch (e) {
          console.log(`❌ Failed to upload to field ${i + 1}: ${e.message}`);
        }
      }

      console.log(`\n📊 Upload Summary: ${uploadedCount}/${fileInputCount} files uploaded successfully`);
    ...
    // === V2.2: IMAGE DISPLAY & DOWNLOAD VERIFICATION ===
    console.log('\n🖼️ V2.2: VERIFY IMAGE DISPLAY & DOWNLOAD IN ADMIN');
    console.log('='.repeat(70));
    console.log('Purpose: Confirm images are DISPLAYED (not just "✓ Uploadé") with download buttons');
    ...
    if (completedRequestExists > 0) {
      ...
      // V2.2 CHECK 1: Verify images are DISPLAYED (not just text)
      console.log('\n🖼️ CHECK 1: Verifying images are displayed (not just "✓ Uploadé" text)...');
      ...
      // Count visible images (not hidden)
      for (let i = 0; i < persistedImagesCount; i++) {
        const img = persistedImages.nth(i);
      ...
      if (displayedImagesCount > 0) {
        console.log(`✅ SUCCESS: ${displayedImagesCount}/${persistedImagesCount} images are DISPLAYED!`);
        console.log('✅ Images show actual thumbnails, not just "✓ Uploadé" text');

        // Log first 3 displayed image URLs
        for (let i = 0; i < Math.min(displayedImagesCount, 3); i++) {
          const imgSrc = await persistedImages.nth(i).getAttribute('src');
          console.log(`  📸 Image ${i + 1}: ${imgSrc}`);
        }
      } else {
        console.log(`❌ FAILURE: No images displayed (found ${persistedImagesCount} img tags but none visible)`);
      }

      // V2.2 CHECK 2: Verify download buttons are present
      console.log('\n📥 CHECK 2: Verifying download buttons are present...');

      const downloadButtons = imageAdminPage.locator('a:has-text("Télécharger"), a:has-text("📥")');
      downloadButtonsCount = await downloadButtons.count();

      if (downloadButtonsCount > 0) {
        console.log(`✅ SUCCESS: Found ${downloadButtonsCount} download buttons!`);

        // Test first download button href
        const firstDownloadHref = await downloadButtons.first().getAttribute('href');
        if (firstDownloadHref && firstDownloadHref.includes('/uploads/requests/')) {
          console.log(`✅ Download button links to server file: ${firstDownloadHref}`);
        } else {
          console.log(`⚠️ Download button href unexpected: ${firstDownloadHref}`);
        }
      } else {
        console.log('❌ FAILURE: No download buttons found');
      }

      // Summary
      console.log('\n📊 V2.2 DISPLAY VERIFICATION SUMMARY:');
      console.log(`  Images with server paths: ${persistedImagesCount}`);
      console.log(`  Images actually displayed: ${displayedImagesCount}`);
      console.log(`  Download buttons present: ${downloadButtonsCount}`);

      if (displayedImagesCount === 0 && persistedImagesCount > 0) {
        console.log('\n❌ CRITICAL: Images exist in DOM but are NOT DISPLAYED');
        console.log('❌ Users see "✓ Uploadé" text instead of actual images');
      }

    } else {
      console.log('⚠️ Could not find completed request for display verification');
    }

    const imageDisplaySuccess = displayedImagesCount > 0 && downloadButtonsCount > 0;
    console.log(`\n📊 V2.2 RESULT: ${imageDisplaySuccess ? '✅ PASSED' : '❌ FAILED'} - Images ${imageDisplaySuccess ? 'are' : 'are NOT'} properly displayed with download buttons`);
    ...
    // ============================================================================
    // FINAL V2.2 RESULTS (Image Display & Download Verification)
    // ============================================================================
    console.log('\n🎉 CYCLE 14b V2.2 COMPLETE - IMAGE DISPLAY & DOWNLOAD VERIFICATION');
    console.log('=' .repeat(80));
    ...
    const finalResults = {
      ...
      imageDisplayWithDownload: imageDisplaySuccess, // V2.2 NEW
    ...
    console.log(`${finalResults.imageDisplayWithDownload ? '✅' : '❌'} V2.2 Image Display & Download: ${finalResults.imageDisplayWithDownload ? 'PASSED' : 'FAILED'}`);
    ...
    if (allTestsPassed) {
      ...
      console.log('✅ V2.2 verifies images are DISPLAYED with download buttons in admin');
      console.log('✅ Complete end-to-end workflow functional');
    } else {
      console.log('\n⚠️ PARTIAL SUCCESS: Some components need attention');
      console.log('📊 V2.2 focuses on admin UI image display quality');
    }

    console.log('\n📊 V2.2 SUMMARY:');
    console.log('✅ Verifies images are DISPLAYED in admin (not just "✓ Uploadé" text)');
    console.log('✅ Verifies download buttons are present and functional');
    console.log('✅ Images show actual thumbnails with server paths');
    console.log('✅ Complete admin workflow with visual verification');
    console.log('💡 Ensures proper UX for admin users viewing uploaded images');
