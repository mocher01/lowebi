...
const BlogPage = () => {
  ...
  // Chargement des articles avec optimisation de performance
  ...
  useEffect(() => {
    const loadArticles = async () => {
      try {
        // Démarrer avec les articles fallback pour affichage immédiat
        const fallbackArticles = getEnrichedArticles();
        setArticles(fallbackArticles);
        setIsLoading(false);
        
        // Puis essayer de charger les vrais articles en arrière-plan
        const blogArticles = await getBlogArticles();
        if (blogArticles.length > 0) {
          setArticles(blogArticles);
          console.log('⚡ Articles Markdown chargés avec succès');
        }
      } catch (error) {
        console.error('Error loading blog articles:', error);
        // Les articles fallback sont déjà chargés
        setIsLoading(false);
      }
    };
