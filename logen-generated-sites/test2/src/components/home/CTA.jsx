...
const CTA = () => {
  ...
  const getSmartCTA = () => {
    ...
    // Use configurable CTA content instead of hardcoded learning-focused text
    const ctaConfig = config.content?.cta || {};
    
    if (isEducational || isCreative) {
      return {
        title: ctaConfig.title || "Prêt à démarrer votre projet ?",
        description: ctaConfig.description || `Contactez-nous dès aujourd'hui pour en savoir plus sur nos ${serviceName} et découvrir comment nous pouvons vous accompagner.`,
        secondaryButton: ctaConfig.secondaryButton || `Découvrir nos ${serviceName}`,
        secondaryPath: servicePath
      };
    } else {
      return {
        title: ctaConfig.title || "Prêt à commencer votre projet ?",
        description: ctaConfig.description || "Contactez-nous dès aujourd'hui pour discuter de votre projet et découvrir comment nous pouvons vous aider à atteindre vos objectifs.",
        secondaryButton: ctaConfig.secondaryButton || `Découvrir nos ${serviceName}`,
        secondaryPath: servicePath
      };
    }
